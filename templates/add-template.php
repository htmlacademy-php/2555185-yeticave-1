<form class="form form--add-lot<?= ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($errors)) ? ' form--invalid' : '' ?>" action="add.php" method="post" enctype="multipart/form-data">
    ...
    <div class="form__container-two">
        <div class="form__item<?= ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($errors['lot-name'])) ? ' form__item--invalid' : '' ?>">
            ...
            <input id="lot-name" type="text" name="lot-name" placeholder="Введите наименование лота" value="<?= htmlspecialchars(getPostVal('lot-name')) ?>">
        ...
        <div class="form__item<?= ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($errors['category'])) ? ' form__item--invalid' : '' ?>">
            ...
            <select id="category" name="category">
                ...
                <?php foreach ($categories as $item): ?>
                    <option value="<?= htmlspecialchars($item['id']) ?>" <?= (getPostVal('category') == $item['id']) ? 'selected' : '' ?>>
    ...
    <div class="form__container-three">
        <div class="form__item form__item--small<?= ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($errors['lot-rate'])) ? ' form__item--invalid' : '' ?>">
            ...
            <input id="lot-rate" type="text" name="lot-rate" placeholder="0" value="<?= htmlspecialchars(getPostVal('lot-rate')) ?>">
        ...
        <div class="form__item form__item--small<?= ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($errors['lot-step'])) ? ' form__item--invalid' : '' ?>">
            ...
            <input id="lot-step" type="text" name="lot-step" placeholder="0" value="<?= htmlspecialchars(getPostVal('lot-step')) ?>">
        ...
        <div class="form__item <?= ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($errors['lot-date'])) ? ' form__item--invalid' : '' ?>">
            ...
            <input class="form__input-date" id="lot-date" type="text" name="lot-date" placeholder="Введите дату в формате ГГГГ-ММ-ДД" value="<?= htmlspecialchars(getPostVal('lot-date')) ?>">
